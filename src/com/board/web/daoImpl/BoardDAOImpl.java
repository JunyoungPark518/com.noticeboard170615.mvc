...
/*	public static void main(String[] args) {
	...
	public List<ArticleBean> list(Map<String, Object> paramMap){
		...
		try {
			...
			String sql="SELECT * FROM (SELECT @NO := @NO + 1 AS ROWNUM, A.* FROM ( SELECT * FROM Board) A,( SELECT @NO := 0 ) B ORDER BY seq_no DESC) C WHERE C.ROWNUM BETWEEN "+s+" AND "+e+"";
